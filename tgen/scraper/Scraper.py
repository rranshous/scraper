...
class Client(Iface):
  ...
  def gen_sitemap(self, root_url):
    ...
    Parameters:
     - root_url
    """
    self.send_gen_sitemap(root_url)
    return self.recv_gen_sitemap()

  def send_gen_sitemap(self, root_url):
    self._oprot.writeMessageBegin('gen_sitemap', TMessageType.CALL, self._seqid)
    args = gen_sitemap_args()
    args.root_url = root_url
    args.write(self._oprot)
    self._oprot.writeMessageEnd()
    self._oprot.trans.flush()

  def recv_gen_sitemap(self, ):
    (fname, mtype, rseqid) = self._iprot.readMessageBegin()
    if mtype == TMessageType.EXCEPTION:
      x = TApplicationException()
      x.read(self._iprot)
    ...
    if result.ex != None:
      raise result.ex
    raise TApplicationException(TApplicationException.MISSING_RESULT, "gen_sitemap failed: unknown result");
...
class Processor(Iface, TProcessor):
  def __init__(self, handler):
    ...
    self._processMap["gen_sitemap"] = Processor.process_gen_sitemap
  ...
  def process_gen_sitemap(self, seqid, iprot, oprot):
    args = gen_sitemap_args()
    args.read(iprot)
    iprot.readMessageEnd()
    result = gen_sitemap_result()
    try:
      result.success = self._handler.gen_sitemap(args.root_url)
    except Exception, ex:
      result.ex = ex
    oprot.writeMessageBegin("gen_sitemap", TMessageType.REPLY, seqid)
    result.write(oprot)
    oprot.writeMessageEnd()
    oprot.trans.flush()
...
class gen_sitemap_args:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('gen_sitemap_args')
    if self.root_url != None:
      oprot.writeFieldBegin('root_url', TType.STRING, 1)
      oprot.writeString(self.root_url)
      oprot.writeFieldEnd()
...
class gen_sitemap_result:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('gen_sitemap_result')
    if self.success != None:
